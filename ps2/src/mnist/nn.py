...
def softmax(x):
    ...
    # std_array = np.std(x)
    # x_normalized = (x - mean_array)/std_array
...
def sigmoid(x):
    ...
    # *** START CODE HERE ***
    return 1 / (1 + np.exp(-x))
    # array = np.zeros_like(x).astype(int)
...
def backward_prop(data, labels, params, forward_prop_func):
    ...
    b2 = params["b2"]
    # Forward pass
    a_hidden, out, loss = forward_prop_func(data, labels, params)
    delta2 = (out - labels).reshape(N, K)
    delta1 = (delta2 @ W2.T) * (a_hidden * (1 - a_hidden))
    
    dw2 = a_hidden.T @ delta2
    dw1 = data.T @ delta1
...
def gradient_descent_epoch(train_data, train_labels, learning_rate, batch_size, params, forward_prop_func, backward_prop_func):
    ...
    split = np.linspace(N, batch_size).astype(int)[::-1]     # 创建等差数列,  [::-1]顺序相反操作
    for index in split:
        # each mini batch
