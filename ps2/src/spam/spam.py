...
def fit_naive_bayes_model(matrix, labels):
    ...
    Args:
        matrix: A numpy array containing word counts for the training data (4457, 1758)
        labels: The binary (0 or 1) labels for that training data (1758, )
    ...
    for y in (0, 1):
    	...
    	phi_k[y, :] = prob
    phi_y[0] = np.mean(labels == 0)
    phi_y[1] = np.mean(labels == 1)
    model['vocab'] = phi_k
    model['class'] = phi_y
    return model
    # *** END CODE HERE ***
...
def predict_from_naive_bayes_model(model, matrix):
    ...
    N, V = matrix.shape
    phi_k, phi_y = model["vocab"], model['class']
    probs = np.zeros((N, 2))
    for i in range(N):
        word = matrix[i, :] # the word representation
        for y in (0, 1):
            probs[i, y] = np.log(phi_y[y]) + np.dot(np.log(phi_k[y, :]), word)
...
def get_top_five_naive_bayes_words(model, dictionary):
    ...
    for word, index in dictionary.items():
    	score = np.log(phi_k[1, index] / phi_k[0, index])
    	influence.append((word, score))
    influence.sort(key=lambda x: -x[1]) # 默认是升序的，所以要有负号
